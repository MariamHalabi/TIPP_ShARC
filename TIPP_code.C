...
TGraph* gKline; //where the calculated data is stocked
  //Limits for minimization
double gLimitXmin = -1 ;
double gLimitXmax = +1 ;
double gLimitYmin = -1 ;
double gLimitYmax = +1 ;
double gLimitZmin = -1 ;
double gLimitZmax = +6 ; //to replace it by +3 if the file is "training_23Nedd_TIPP_Zm2_Shift.root"
...
void TIPP_code(TString name ,const Int_t method , const Int_t Det_selected,const Int_t Strip_selected)
{
  ...
  {
    nb_pixels = 48;  //we have 48 back strips
  }
  else if (method == 2) //back strip selected
        {
        nb_pixels = 24;  //we have 24 front strips
        }
        else
                {
 ...
 //DISPLAY
  ...
  //Fill energy , energy std , theta, and theta std vectors by values (for shifted and initial values)
   ...
   if (gmethod == 1) //selected front strip
    {
      if (Det==Det_selected && Front==Strip_selected)
      {
       ...
       add_elements(&Strip_big_vector, Back-1, x, y, z, ELab, Theta_calc); //fill the selected front strip big vector with values
      }
    }
    if (gmethod == 2){
      ...
      {
       Theta_calc = theta_cal(x,y,z); //Compute the angle
        add_elements(&Strip_big_vector, Front-1, x, y, z, ELab, Theta_calc); //fill the selected back strip big vector with values 
      }
    } 
   }
